...
for needle,label in [("const page=document.getElementById('bookPage')",'visible-page height source'),('Math.max(page.scrollHeight,page.offsetHeight,r?r.height:0)','visible-page height'),('window.reportHostHeight=window.reportAndroidContentHeight','height override alias')]:
 if needle not in idx:errs.append(label)
if 'new ScrollView(this)' not in java:errs.append('native ScrollView')
if 'setNestedScrollingEnabled(false)' not in java:errs.append('WebView nested scrolling')
nav_ok=(('currentBookIndex' in java and 'goToPage(currentBookIndex + 1)' in java and 'goToPage(currentBookIndex - 1)' in java) or ('bookIndex+1' in java and 'bookIndex-1' in java))
if not nav_ok:errs.append('native previous/next navigation')
visual_ok=('nativeToggleVisual' in java or 'toggleVisualFocusNative()' in java or 'visualButton.setOnClickListener' in java)
if not visual_ok:errs.append('visual action')
